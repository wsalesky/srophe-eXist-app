...
function api:search-element($element as xs:string?, $q as xs:string*, $collection as xs:string*, $lang as xs:string*, $author as xs:string*){
    ...
    let $collection := if($config:get-config//repo:collection[@name = $collection]) then
                            string($config:get-config//repo:collection[@name = $collection]/@name)
                       else if($config:get-config//repo:collection[@title = $collection]) then
                            string($config:get-config//repo:collection[@title = $collection]/@name)
                       else ()
